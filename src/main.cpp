...
// All sorts of variables/constants need declaration
...
String chopZeros(String bigNumber)
{
  ...
  {
    while (bigNumber.endsWith("0"))
      bigNumber.remove(bigNumber.length() - 1);
    if (bigNumber.endsWith("."))
      bigNumber.remove(bigNumber.length() - 1);
  }
  return bigNumber;
}
...
float_t humanFreqToFloat(String newFreq)
{
  ...
  if (newFreq.endsWith("k") || newFreq.endsWith("m"))
  {
    ...
    if (newFreq.endsWith("k"))
      newFreq = newFCMD.toFloat() * 1000;
    if (newFreq.endsWith("m"))
      newFreq = newFCMD.toFloat() * 1000000;
  }
  ...
  */
  ...
  for (uint8_t div = 0; div < 8; div++)
  {
    ...
    if ((s > 0) && ((div == 0) || (s < 1024)))
    {
      ...
      if (delta < delta_min)
      { // Deviation is less! -> Store current values..
        step = s;
        divi = div;
        delta_min = delta;
      }
  ...
  if (eXi)
    Serial.printf(" returning frequency: %.2f \n", frequency);
...
void frequencyStepSet(String newFreq)
{
  ...
  if (thisFreq < 1)
    thisFreq = 1; // 1Hz
  if (thisFreq > 10000000)
    thisFreq = 10000000; // 10MHz
...
void frequencySet(String newFreq, bool OVRide = false, bool doReport = true)
{
  ...
  if (eXi && doReport)
    Serial.printf(" User requested frequency:\t%s\n", makeHumanFrequency(frequency).c_str());
  if (OVRide == false)
    checkLimits(frequency);
  prefs.putFloat("f", frequency);
  if (eXi && doReport)
    Serial.printf(" Setting frequency:\t%s\n", makeHumanFrequency(frequency).c_str());
}
...
void frequencyStepUP(int8_t stepSource)
{
  ...
  if (eXi)
    Serial.printf(" Step UP() activation from: %s\n", stepFROM(stepSource).c_str());
  frequency += fStep;
  if (stepsObeyLimits)
    checkLimits(frequency);
...
void frequencyStepDOWN(int8_t stepSource)
{
  ...
  if (eXi)
    Serial.printf(" Step DOWN() activation from: %s\n", stepFROM(stepSource).c_str());
  frequency -= fStep;
  if (stepsObeyLimits)
    checkLimits(frequency);
  if ((stepSource == 0 && storeTouch == true) || (stepSource == 1 && storeButton == true))
...
{
  if (voltsValue > 3.3)
    Serial.printf(" Value exceeds 3.3! Will be automatically limited to 3.3\n");
...
String copyPreset(uint8_t copyFrom, uint8_t copyTo)
{
  ...
  if (copyFrom < 1 || copyFrom > presetMAX)
    return "Source " + _OUT_OF_RANGE_;
  if (copyTo < 1 || copyTo > presetMAX)
    return "Target " + _OUT_OF_RANGE_;
  ...
  if (prefsSwitch(copyFrom))
  {
    ...
    if (Cmode != '~')
      prefs.putChar("m", Cmode);
    if (Cfrequency != -1)
      prefs.putFloat("f", Cfrequency);
    if (CfStep != -1)
      prefs.putFloat("s", CfStep);
    if (CtouchMode != '~')
      prefs.putChar("h", CtouchMode);
    if (CwaveAmplitude != '~')
      prefs.putUChar("a", CwaveAmplitude);
    if (CPWMResBits != '~')
      prefs.putUChar("b", CPWMResBits);
    if (Cpulse != '~')
      prefs.putUChar("p", Cpulse);
    if (CpStep != '~')
      prefs.putUChar("j", CpStep);
    if (CloopCommands != "")
      prefs.putString("o", CloopCommands);
    ...
    if (CmyName != "")
    {
      ...
      if (foundCpyPOS != -1)
      { // And here it is again using String functions..
...
String savePreset(int8_t presetNumber, bool defaults = false)
{
  ...
  if (layerPresets && !defaults)
  {
    ...
    if (lMode != '~')
      prefs.putChar("m", lMode);
    if (lFreq != -1)
      prefs.putFloat("f", lFreq);
    if (lStep != -1)
      prefs.putFloat("s", lStep);
    if (lTcMd != '~')
      prefs.putChar("h", lTcMd);
    if ((mode != 'r' || saveALL) && lAMP != '~')
      prefs.putUChar("a", lAMP);
    if ((mode == 'r' || saveALL) && lBits != '~')
      prefs.putUChar("b", lBits);
    if ((mode != 's' || saveALL) && lPulse != '~')
      prefs.putUChar("p", lPulse);
    if ((mode != 's' || saveALL) && lpStep != '~')
      prefs.putUChar("j", lpStep);
  }
  else
  {
...
String namePreset(String presetNameData)
{
  ...
  for (uint8_t i = 0; i < presetNameData.length(); i++)
  {
    ...
    if (i == 2)
      stillDigits = false; // Something more robust would be easy to code. Or remove this line.
  ...
  if (success == -1)
  {
    ...
    if (prefs.getChar("i", -1) != -1)
    {
      if (prefs.putString("n", presetNAME) != 0)
        success = 0;
    }
    else
    {
...
String getCurrentSettings()
{
  ...
  if (mode != 's')
    sprintf(buffer + strlen(buffer), "\tPulse Width: %i%%\n", pulse);
  // Resolution should not matter for Triangle wave. No really. Hmm.
  if (mode == 'r')
    sprintf(buffer + strlen(buffer), "\tPWM Resolution: %i bit%c\n", PWMResBits, bpl);
  sprintf(buffer + strlen(buffer), "\tFreq Step Size: %s\n", makeHumanFrequency(fStep).c_str());
  if (mode != 's')
    sprintf(buffer + strlen(buffer), "\tPWM Step Size: %i%%\n", pStep);
  if (mode != 'r')
    sprintf(buffer + strlen(buffer), "\tAmplitude Level: %i\n", waveAmplitude);
...
String getNVRAMPresetData(uint8_t presetNumber = 0, bool listing = false, bool silent = false)
{
  ...
  if (presetNumber != 0)
  {
    ...
    if (!silent)
    {
      ...
      {
        if (!listing)
          LastMessage = "Preset " + (String)presetNumber + " does not exist";
        prefsSwitch();
        return "";
      }
  ...
  if (!silent)
  { // as the array is still there in the stack.
    if (presetNumber != 0)
    {
      if (!presetEmpty())
      {
    ...
    if (eXi && !listing)
      sprintf(nvbuf + strlen(nvbuf), getFreeEntries().c_str());
  }
...
String listPresets(bool init = false)
{
  ...
  for (uint8_t i = 1; i <= presetMAX; i++)
    xMSG += getNVRAMPresetData(i, true, init);
  xMSG = (xMSG != "") ? "Listing Preset Memories..\n" + xMSG : "No Presets Exist!\n";
  if (!init)
    return xMSG + getFreeEntries();
...
String listLoops(bool oneLine)
{
  ...
  for (uint8_t i = 1; i <= presetMAX; i++)
  {
    ...
    if (sLoops != "")
      xMSG += " loop" + (String)i + "=" + sLoops + loopEnd;
  }
...
#include "WebPage.h"
  ...
  */
  ...
  if (eXi)
    Serial.printf(" HTTP Request: Main Page for client @ %s\n",
...
void handleFreqencyChange()
{
  ...
  if (eXi)
    Serial.println(" HTTP Request (Frequency Change): " + WebCommand);
}
...
void handleStepChange()
{
  ...
  if (eXi)
    Serial.println(" HTTP Request (Step Size Change): " + WebCommand);
}
...
void handleUpdateWaveMode()
{
  ...
  if (eXi)
    Serial.println(" HTTP Request: Get Waveform");
}
...
void handleUpdateFrequency()
{
  ...
  if (eXi)
    Serial.println(" HTTP Request: Get Frequency");
}
...
void handleBitDepthChange()
{
  ...
  if (eXi)
    Serial.printf(" HTTP Request: Bit Depth. %s bit%c\n", iData.c_str(), bpl);
}
...
void handlePulseWidthChange()
{
  ...
  if (eXi)
    Serial.println(" HTTP Request:" + WebCommand);
}
...
void handleFreqencyUP()
{
  ...
  if (eXi)
    Serial.printf(" HTTP Request: Frequency UP to %s\n", mf.c_str());
}
...
void handleFreqencyDOWN()
{
  ...
  if (eXi)
    Serial.printf(" HTTP Request: Frequency DOWN to %s\n", mf.c_str());
}
/*
...
void handlePulseUP()
{
  ...
  if (eXi)
    Serial.printf(" HTTP Request: Pulse Width UP to %i\n", pulse);
}
...
void handlePulseDOWN()
{
  ...
  if (eXi)
    Serial.printf(" HTTP Request: Pulse Width DOWN to %i\n", pulse);
}
...
void handleResUP()
{
  ...
  if (eXi)
    Serial.printf(" HTTP Request: Resolution Bit Depth UP to %i\n", PWMResBits);
}
/*
/resDOWN             */
void handleResDOWN()
{
  ...
  if (eXi)
    Serial.printf(" HTTP Request: Resolution Bit Depth DOWN to %i\n", PWMResBits);
}
...
void handleStatus()
{
  ...
  if (eXi)
    Serial.println(" HTTP Request: Get Current Settings.");
...
void handleEndLoop()
{
  endLoop();
  if (eXi)
    Serial.println(" HTTP Request: End Loop.");
...
void handleLoadPreset()
{
  ...
  if (eXi)
    Serial.println(" HTTP Request: " + state);
}
...
void handleWebConsole()
{
  ...
  if (eXi)
    Serial.printf(" HTTP Request: WebCommand: %s\n", WebCommand.c_str());
}
...
#include "Console.h"
  ...
  if (eXi)
    Serial.printf(" HTTP Request: WebConsole for client @ %s\n", myClient.c_str());
}
...
void sendSimplePage()
{
  ...
  */
  ...
  if (eXi)
    Serial.printf(" HTTP Request: Simple Page for client @ %s\n",
                  server.client().remoteIP().toString().c_str());
}
...
void sendListPage()
{
  ...
  if (eXi)
    Serial.println(" HTTP Request: List Presets");
}
...
void sendListPageHTML()
{
  ...
  if (eXi)
    Serial.println(" HTTP Request: List Presets (HTML)");
}
...
void sendHelpPage()
{
  ...
  server.send(200, _PLAIN_TEXT_, LastMessage);
  if (eXi)
    Serial.println(" HTTP Request: help");
}
...
void sendHelpPageHTML()
{
  ...
  server.send(200, _PLAIN_TEXT_, "<!DOCTYPE html><pre>" + LastMessage);
  if (eXi)
    Serial.println(" HTTP Request: help (HTML)");
}
  ...
  while ((thisPref = strsep(&imported, ",")))
  {
    ...
    { // Default settings..
      ...
      case 'p':
        ...
        break;
      case 'a':
        if (setAmplitude(thisINT))
          sprintf(iBuff + strlen(iBuff), "\tSetting Amplitude Level to: %i\n", thisINT);
        break;
      case 'j':
        if (setPulseStep(thisINT))
          sprintf(iBuff + strlen(iBuff), "\tSetting Pulse Width Step to: %i%%\n", thisINT);
        break;
      case 'b':
...
// We need to define the function prototype here, otherwise it won't compile. Chicken and egg type of situation ...
String exportSettings(String exData, bool all);
/*
...
String exportALLSettings()
{
  ...
  for (uint8_t i = 1; i <= presetMAX; i++)
  {
    ...
    if (prefsSwitch(i))
    {
      ...
      if (lD != "")
        loopData += commandDelimiter + " loop" + (String)i + "=" + lD;
    }
  ...
  if (curSettings != "")
    data += "w" + commandDelimiter + curSettings;
  lD = prefs.getString("o", "");
  if (lD != "")
    loopData = commandDelimiter + " loop=" + lD + loopData;
...
See the exportALLSettings(below) for more details.
                                   ...
                                   */
String exportSettings(String exData = "0", bool all = false)
{ // "all" is true during an "export all" command

  if (exData.indexOf("all") != -1)
    return exportALLSettings(); // We'll be back!
  ...
  if (presetNUM < 0 || presetNUM > presetMAX)
    success = -1;
  ...
  if (success == 0)
  {
    ...
    if (success == 0)
    {
      ...
      if (prefs.getChar("m", '~') != '~')
        sprintf(eBuff, "m=%c,", prefs.getChar("m"));
      if (prefs.getFloat("f", -1) != -1)
        sprintf(eBuff + strlen(eBuff),
                "f=%s,", makeHumanFrequency(prefs.getFloat("f"), true).c_str());
      if (prefs.getUChar("p", '~') != '~')
        sprintf(eBuff + strlen(eBuff), "p=%i,", prefs.getUChar("p"));
      if (prefs.getUChar("b", '~') != '~')
        sprintf(eBuff + strlen(eBuff), "b=%i,", prefs.getUChar("b"));
      if (prefs.getFloat("s", -1) != -1)
        sprintf(eBuff + strlen(eBuff), "s=%s,",
                makeHumanFrequency(prefs.getFloat("s"), true).c_str());
      if (prefs.getUChar("j", '~') != '~')
        sprintf(eBuff + strlen(eBuff), "j=%i,", prefs.getUChar("j"));
      if (prefs.getUChar("a", '~') != '~')
        sprintf(eBuff + strlen(eBuff), "a=%i,", prefs.getUChar("a"));
      if (prefs.getChar("h", '~') != '~')
        sprintf(eBuff + strlen(eBuff), "h=%c,", prefs.getChar("h"));
  ...
  else if (loopData == "" && success != -2)
    success = -3; // no data and not a preset
...
String setupPhysicalButtons(bool reportOnly = true)
{
  ...
  if (useButtons)
  {
    ...
    for (uint8_t i = 0; i < buttonCount; i++)
    {
      ...
      {
        if (!reportOnly)
        { // It wouldn't be a problem to re-assign, but still.
...
void loop()
{
  ...
  if (!buttonPressed)
  {
    ...
    if (amDelaying)
    {
      if (currentTime < delayStart + delayTime)
        return;
  ...
  if ((Serial.available() > 0) || (WebCommand != "") || QCommand != "")
  {
    ...
    if (eqPos != -1 && eqPos < 11 && raw.substring(0, 4) == "loop")
    {
      ...
      else if (loopCommands)
      {
        ...
        {
          LastMessage = (prefs.putString("o", loopCommands)) ? "Saved loop commands" : "Failed to save loop commands";
        }
      }
    ...
    if (cmd == '*' && cmdData != "")
    {
      ...
      startSignal("Music");
      if (isSerial && eXi && QCommand == "")
        Serial.printf(LastMessage.c_str());
      return;
    }
    ...
    if (input.substring(0, 6) == "remote")
    {
      ...
      if (isSerial || eXi)
        Serial.printf("Remote Control is  %s\n", xMSG.c_str());
    ...
    if ((input == "x") || input == "reboot")
    {
      ...
      if (QCommand != "")
      {
        ...
        else
        {
          ...
          if (prefs.putString("q", QCommand))
            Serial.println(" Queued Commands Stored: OK");
        }
      }
    ...
    if (input == "buttons")
    {
      ...
      if (isSerial || eXi)
        Serial.printf("%s", LastMessage.c_str());
      return;
    }
    ...
    if (input.substring(0, 4) == "loop")
    {
      ...
      if (loopCommands != "")
      {
        ...
        while (loopCommands.endsWith(commandDelimiter))
          loopCommands.remove(loopCommands.length() - 1);
    ...
    /*
    Memory information..               */
    if (input.substring(0, 3) == "mem")
    { // "memory information please" will also work.
      char mbuf[380];
      ...
      */
      case 'w':
        ...
        else
        {
          ...
          {
          case 3:
            if (!isDigit(cmdData.charAt(2)))
              doWipe = false;
            // fall through
          case 2:
            if (!isDigit(cmdData.charAt(1)))
              doWipe = false;
            // fall through
          case 1:
            if (!isDigit(cmdData.charAt(0)))
              doWipe = false;
    ...
    if (!iLooping && (fChange || mChange || pChange || bChange || tChange || jChange || vChange || aChange))
    {
      ...
      if (mChange)
        sprintf(buffer, " Generator set to: %s Wave\n", makeHumanMode(mode).c_str());
      if (fChange)
      {
        if (didLimit)
        {
      ...
      if (pChange)
        sprintf(buffer + strlen(buffer), " Pulse Width set to: %i%s\n", pulse, rState.c_str());
      if (tChange)
        sprintf(buffer + strlen(buffer), " Frequency Step set to: %s\n", makeHumanFrequency(fStep).c_str());
      if (jChange)
        sprintf(buffer + strlen(buffer), " PWM Step set to: %i\n", pStep);
      if (vChange != "")
        sprintf(buffer + strlen(buffer), " Amplitude set to: %s\n", vChange.c_str());
      if (aChange)
        sprintf(buffer + strlen(buffer), " Potentiometer Accuracy set to: %i\n", stepAccuracy);
      if (bChange)
      {
        char bpl;
        if (PWMResBits != 1)
      ...
      if (loadedPreset != "")
        sprintf(buffer + strlen(buffer), " Loaded Preset %s\n", loadedPreset.c_str());
      if (savedPreset != "")
        sprintf(buffer + strlen(buffer), " Saved Preset %s\n", savedPreset.c_str());
    ...
    if (isSerial)
      Serial.println("\n Current Settings:\n");
    if (isSerial)
      Serial.println(getCurrentSettings());
  }
  ...
  { // 24h in milliseconds == (24 * 60 * 60 * 1000) !
    if (eXi)
      Serial.println("\n Scheduled Reboot..");
  }
}
